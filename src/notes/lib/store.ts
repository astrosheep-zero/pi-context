...
import { assertAddress, assertGlobPattern, assertVirtualPath, addressFor, globToRegExp } from "./address.js";
...
export type ListOptions = { scope?: Scope; who?: string; pattern?: string };
...
export type NoteEditResult = { meta: NoteMeta; applied: number; resolved_scope: Scope; change: { before: string; after: string } };
...
export interface NotesStore {
	...
	list(options?: ListOptions): NoteRow[];
	search(queries: string[], options?: SearchOptions): NoteSearchRow[];
}
...
function homesFor(context: NotesContext, opts: ListOptions): HomeRef[] {
...
export function createNotesStore(input: NotesContext): NotesStore {
	...
	function edit(address: string, edits?: EditOperation[], options: EditOptions = {}): NoteEditResult {
		...
		const change = bodyChanged && metadataChanged
			? { before: raw, after: serialized }
			: bodyChanged
				? { before: body, after: next }
				: metadataChanged
					? { before: frontmatterOf(beforeMeta), after: frontmatterOf(meta) }
					: { before: "", after: "" };
		atomicWrite(path, serialized);
		return { meta, applied: operations.length, resolved_scope: scope, change };
	}

	function list(options: ListOptions = {}): NoteRow[] {
	...
	function search(queries: string[], options: SearchOptions = {}): NoteSearchRow[] {
		const matcher = matcherFor(normalizePattern(options.pattern, context));
