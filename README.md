...
- **`wipe_memory`** — the model can request a fresh context window after completing a tool batch. Manual `/wipe-memory` requests a reset at the next completed turn end: when idle it sends a hidden warning to start that turn; during output it schedules the boundary on the running turn and acknowledges the request immediately. The budget warning still starts a multi-turn close-out; a successful normal stop falls back to a reset. Abort/error never counts as completion. Raw conversation remains in the session and history_* tools, but is excluded from the next provider context.
...
`/wipe-memory` acknowledges the request immediately and commits at the next successful `turn_end`, after the current assistant response and its complete tool batch. When idle it sends a hidden warning to start one ordinary model turn; during output it does not send a warning because Pi would queue it for a later turn, possibly after the reset. A tool turn may continue automatically in the fresh window. Abort/error does not commit. The budget warning still arms a multi-turn close-out: explicit `wipe_memory` commits after its batch, or a successful normal stop commits after queued messages drain. The hard reserve remains a separate safety boundary. Direct `wipe_memory` remains valid without a prior warning. While pi-context is enabled, `/compact` is cancelled with an actionable `/wipe-memory` notice. Budget close-outs wait for queued steering/follow-up work in the current window; turn-end resets can deliver queued work in the fresh window exactly once. An aborted turn does not manufacture a continuation.
