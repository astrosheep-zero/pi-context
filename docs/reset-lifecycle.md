...
- **Process interruption.** If Pi stops between the marker and the later reset messages, the persisted tail is a marker and metadata only. Nothing is rewritten or reinterpreted on the next start.
